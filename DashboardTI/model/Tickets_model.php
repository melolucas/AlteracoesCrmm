...
class Tickets_model extends Crud_model {
    ...
    public function updateAvaliacao($id, $estrela, $observacao) {
        $sql = "UPDATE `tickets` SET `avaliacao`= '{$estrela}', `observacao`= '{$observacao}' WHERE `id`= {$id}";
        $this->db->query($sql);
        
        return true;
    }

    public function selectAvaliacao($id) {
        $sql = "SELECT avaliacao, observacao FROM tickets WHERE id = {$id}";
        $avaliacao = $this->db->query($sql);
        return $avaliacao->result();
    }

    public function selectRanking($dataInicio, $dataFinal) {
        if($dataInicio != '') {
            ...
            $where = "AND t.created_at BETWEEN '{$dataIni}' AND '{$dataFin}'";
        }       

        $sql = "SELECT COUNT(id) as rank, assigned_to
                from tickets t
                where ticket_type_id = 1
                and status = 'closed'
                and assigned_to IN(3,22,91,239)
                and closed_at $where 
