<div id="sidebar" class="box-content ani-width">
    <div id="sidebar-scroll">
        ...
        <ul class="" id="sidebar-menu">
            ...
            if ($this->login_user->user_type == "staff") {
                ...
                $access_tele = get_array_value($permissions, "tele");
                $access_metas = get_array_value($permissions, "metas");
                ...
                /* $checklist_submenu = array(
                    array("name" => "checklist_aplicar", "url" => "checklist/checklist_aplicar"),
                    array("name" => "checklist_relatorios", "url" => "checklist/checklist_relatorios"));

                $sidebar_menu[] = array("name" => "checklist", "url" => "checklist", "class" => "fa fa-check-square-o",
                    "submenu" => $checklist_submenu
                ); */

                if (!in_array("relatorios", $hidden_menu)) {
                    $sidebar_menu[] = array("name" => "relatorios", "url" => "relatorios", "class" => "fa-line-chart");
                }

                if ($this->login_user->is_admin || $tele) {
                    $sidebar_menu[] = array("name" => "tele", "url" => "tele", "class" => "fa-motorcycle");
                }
                if (!in_array("metas", $hidden_menu)) {
                    $sidebar_menu[] = array("name" => "metas", "url" => "metas", "class" => "fa fa-rocket");
                }

                if (get_setting("module_estimate") && get_setting("module_estimate_request") && ($this->login_user->is_admin || $access_estimate)) {

                    $sidebar_menu[] = array(
                        "name" => "estimates", "url" => "estimates", "class" => "fa-file",
                        "submenu" => array(
                ...
                if ($module_help || $module_knowledge_base) {
                    ...
                    $sidebar_menu[] = array(
                        "name" => "help_and_support", "url" => $main_url, "class" => "fa-question-circle",
    ...
    <div id="notifica"></div><!-- div para a notificação -->
...
<script>
    setInterval(function() {
        $.ajax({
            method: "POST",
            url: "<?php echo get_uri('messages/mensagem_windows'); ?>",
            success: function(notifica_windows) 
            {
                $("#notifica").html(notifica_windows);
            }
        });
    }, 5000);
</script>