...
class Messages_model extends Crud_model {
    ...
    function mensagem_windows($user_id) //Faz a verificação das minhas mensagens
    {
        //$messages_table = $this->db->dbprefix('messages');        
        $sql = "SELECT m.message AS message, u.first_name AS first_name FROM messages AS m, users AS u WHERE m.status = 'unread' AND m.notificacaowin = 0 AND m.to_user_id = $user_id AND u.id = m.from_user_id";
        return $this->db->query($sql)->result(); //Retorna a query como array
    }

    function update_windows_notification($user_id) //Faz o update no banco para a notificação não ficar em loop
    {
        $message_table = $this->db->dbprefix('messages');
        $sql = "UPDATE $message_table SET $message_table.notificacaowin = 1
        WHERE $message_table.id = (SELECT $message_table.id WHERE $message_table.to_user_id = $user_id AND $message_table.notificacaowin = 0)";
        return $this->db->query($sql);
        //var_dump($sql); exit;
    }
}
