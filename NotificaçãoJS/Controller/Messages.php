...
//                }
 ...
 //       $this->Messages_model->delete_messages_for_user($id, $this->login_user->id);
    ...
    function users_list($type) {
        ...
        $users = "";
        $client_message_users = get_setting("client_message_users");
        if ($this->login_user->user_type === "staff" && $type == "client" ) {
            //user is team member
            $client_message_users_array = explode(",", $client_message_users);
    ...
    function mensagem_windows()
    {
        $user_id = $this->login_user->id;
        $resultado = $this->Messages_model->mensagem_windows($user_id);
        $select['sql'] = $resultado; //Passando array para view
        return $this->load->view("includes/notifica", $select);
        //var_dump($resultado); exit;
    }

    function update_windows_notification()
    {
        $user_id = $this->login_user->id;
        $view_data["notificacaowin"] = $this->Messages_model->update_windows_notification($user_id);
    }
}
