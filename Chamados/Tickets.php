...
class Tickets extends MY_Controller {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($data->status === "closed") {
            ...
            $edit = '<li role="presentation">' . modal_anchor(get_uri("tickets/modal_avaliacao"), "<i class='fa fa-star'></i> " . ' Avaliação', array("title" => ' Avaliação ', "data-post-id" => $data->id, "data-post-view" => "details","data-post-view" => "details","data-post-view" => "details","data-post-view" => "details")) . '</li>';
            $row_data[] = '
                <span class="dropdown inline-block">
                    <button class="btn btn-default dropdown-toggle mt0 mb0" type="button" data-toggle="dropdown" aria-expanded="true">
            ...
            $edit = '<li role="presentation">' . modal_anchor(get_uri("tickets/modal_avaliacao2"), "<i class='fa fa-star'></i> " . ' Avaliação', array("title" => ' Avaliação ', "data-post-view" => "details","data-post-view" => "details","data-post-view" => "details","data-post-view" => "details")) . '</li>';
            $row_data[] = '
                <span class="dropdown inline-block">
                    <button class="btn btn-default dropdown-toggle mt0 mb0" type="button" data-toggle="dropdown" aria-expanded="true">
                        <i class="fa fa-star"></i>&nbsp;
    ...
    // Carrega view do modal Avaliação
    function modal_avaliacao() {
        $id = $this->input->get_post('id');
        $estrela = $this->input->get_post('estrela');
        $observacao = $this->input->get_post('observacao');
        
        if (!empty($id)) {
            $_SESSION['id'] = $id;
            $this->load->model('Tickets_model');
            $selection = $this->Tickets_model->selectAvaliacao($_SESSION['id']);            
            $_SESSION['selection'] = $selection[0];
            $this->load->view('clients/tickets/modal_avaliacao');
        }
        
        if (!empty($estrela)) {
            $this->load->model('Tickets_model');
            $this->Tickets_model->updateAvaliacao($_SESSION['id'], $estrela, $observacao);
            redirect('../index.php/tickets');
        }
    }

    // Carrega view do modal Avaliação 2
    function modal_avaliacao2() {
        $view_data['modal_avaliacao2'] = 'Modal Avaliação 2';
        $this->load->view('clients/tickets/modal_avaliacao2', $view_data);
    }
